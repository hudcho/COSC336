...
\begin{verbatim}
    ...
    H_1(x) then H_2(x)
    Issue: No H_3(x) and no 'i', if we just use H_1(x) then H_2(x) 
           there is no H_3(x) or H_4(x) 
             
    Stops working at key 28, H_2(28)=0 and zero is another collision...

    H_1(x) then H_2(H_1(x))
    Issue: If we use H_1(x) then H_2(H_1(x)). That would allow for a 
            H_3(H_2(H_1(x))) but gets stuck if any return 0
             
    Stops working at 88 as H_1(88)=0 and H_2(0)=0 then H_3(H_2(0))=0...
    ...
    Stops on key 28's issue, H_2(28) = (7-28)mod7 = 0.
    Making no changes even with increasing i's. 
    So H_i(x) will never produce a new index staying on H_i(x)=6 for all i.

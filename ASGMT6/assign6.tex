...
\begin{verbatim}
    ...
    The chapter PPT uses the following
    H_i(x) = (H_1(x) + (i-1) * H_2(x)) mod 11
    i = 0, 1, 2, 3, 4 ...  

    Either way using H_1(x) then H_2(x) or combining them with a incremental i
    Key 28 runs into issues, a collision on either approach. but the issue is...
    
    H_1(28) = 6 
    H_2(28) = 0

    So H_i(x) will never produce a new index staying on H_i(x)=6 for all i.
    and the H_1 then H_2 approach collids again on H_2(28)=0 
\end{verbatim}
